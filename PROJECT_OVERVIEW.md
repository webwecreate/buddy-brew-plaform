...
- [x] **Admin Dashboard ครบทั้ง 7 tab แล้ว** (`docs/admin.html`) — Overview + สมาชิก (รายละเอียดเดิม: login gate เดียวกับ Staff Panel, RLS+grant สำหรับ `authenticated`, `manual_adjust_points()`, photo upload ผ่าน Supabase Storage, `is_favorite`, `tags`) บวกอีก 5 tab ที่เพิ่งทำเสร็จ:
  - **แต้ม** — ledger อ่านอย่างเดียว (ทุกรายการ join ชื่อสมาชิก+เมนู, ค้นหาได้, สรุปแจก/หักวันนี้)
  - **เมนู** — แก้ราคา/แต้ม/เปิดปิดร้อน-เย็น/เปิดขายได้ตรง + เพิ่มเมนู/เมล็ดใหม่ได้ (migration `20260707070000_menu_write_access.sql` เปิด insert/update ให้ `authenticated` แยกจาก `anon` ที่อ่านได้อย่างเดียว)
  - **Badge/Mission** — Badge: toggle เปิดปิดได้ (migration `20260707080000_badge_toggle_access.sql`, column-level grant เฉพาะ `active`) ไม่ให้แก้ condition ตรงๆเพราะ logic unlock ผูกกับโค้ด trigger — **Mission เป็น v1 ที่ตั้งใจตัดขอบเขต**: migration `20260707090000_mission_engine.sql` เพิ่ม `target_count`/`period_start` (แยก mission รายวัน/สัปดาห์แต่ละรอบออกจากกัน) + trigger `check_mission_progress()` ตรวจ progress อัตโนมัติทุกครั้งที่ซื้อของจริง แต่**ยังไม่แจกแต้มอัตโนมัติ**และ**ยังไม่มีหน้า Buddy Book ให้ลูกค้าดู/กด claim mission** — status `completed`→`claimed` (พร้อมแจกแต้มตอนนั้น) เป็นงานฝั่ง Buddy Book ที่จะทำต่อ ไม่ใช่ bug
  - **รายงาน** — ยอดขายแยกเมนู, สัดส่วน Tier, สมาชิกใหม่ 8 สัปดาห์ล่าสุด (คำนวณฝั่ง client ทั้งหมด ไม่มี grant ใหม่)
  - **พนักงาน** — **แค่ดูรายชื่อ account ที่ login ได้ (list-only)** ผ่าน Edge Function ใหม่ `list-staff-accounts` (เรียก Supabase Auth Admin API ด้วย service_role) — **ตั้งใจไม่ทำเพิ่ม/ลบ account ในหน้านี้** เพราะระบบยังไม่มี role แยก staff/admin เลย ทุก account ที่ login ได้เท่ากันหมด ถ้าเปิดให้เพิ่ม/ลบได้ตอนนี้จะเสี่ยงให้ staff คนไหนก็ลบ account เจ้าของร้านได้ — เพิ่ม/ลบ account จริงยังต้องทำผ่าน Supabase Dashboard เอง รอวันที่มีระบบสิทธิ์ค่อยกลับมาเปิดฟีเจอร์นี้
